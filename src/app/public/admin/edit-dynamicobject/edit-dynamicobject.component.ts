...
export class EditDynamicObjectComponent implements OnInit {
  ...
  checkAttributeList(name:string){
    for(var item in this.origionalAttrList){
      if(this.origionalAttrList[item].name==name)
        return true;
    }
    return false;
  }
  createForm() {
    this.formGroupObjectProperties= this.fb.group({
      name: [this.recordData["name"],[Validators.required,CustomValidator.restrictWhiteSpace,Validators.maxLength(30)]],
      desc: [this.recordData["description"],[Validators.required,Validators.maxLength(150)]],
    ...
    this.objectFunctionsGroup  = this.fb.group(this.parseFormControlsFunctionsGroup(this.recordData["functions"]));
