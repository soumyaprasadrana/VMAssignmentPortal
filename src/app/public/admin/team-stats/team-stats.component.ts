...
export class TeamStatsComponent implements OnInit {
  ...
  constructor(private us: UserService,
    ...
    private dialog: MatDialog,private _auth:AuthserviceService,
  ...
  resetPassword(user:String){
    if (
      confirm(`Do you really want to reset password for user   "${user}"?`)
    ){
              ...
              .then((res2: any) => {
                ...
                if (res2.status == 'Success' || res2.status==true) {
                  ...
                  if(this.loggedUser.useToast){
                    this.toastService.showSuccess('User password updated successfully!',5000);
                    this.openDialog(
                      {
                        type: 'message',
                        message: 'Updated password for user: '+user+' is '+atob(res2.newPassword).toString(),
                      },
                      null
                    );
                  }else{
                  this.openDialog(
                    {
                      type: 'message',
                      message: 'User password updated successfully!'+'\n'+'Updated password for user: '+user+' is '+atob(res2.newPassword).toString(),
                    },
                    null
                  );
                  }
