...
export class HomePageComponent implements OnInit {
  columnDef: Column[] = [];
  vmGridOptions: GridOption | any;
  vmDataSet: any[] = [];
  isLoaded: boolean = false;
  isDataloadFailed: boolean = false;
  angularGrid: AngularGridInstance | any;
  editQueue: any[] = [];
  editedItems: any = {};
  isGridEditable = true;
      ...
      .then((res) => {
        ...
        promise
          .then((res: any[]) => {
            ...
            this.vmDataSet = res;
            const owners_list = [
              ...new Set(this.vmDataSet.map((item) => item.owner)),
            ];
            ...
            for (var item in owners_list) {
              if (owners_list[item] != "")
                this.ownerList.push({
                  owner: owners_list[item].toUpperCase(),
                  value: owners_list[item],
                });
            }

            this.ownerList = [
              ...new Map(
                this.ownerList.map((item: any) => [ item["owner"], item ])
              ).values(),
            ];
        ...
        if (res2.status == "Success") {
          if (this.loggedUser.useToast)
            this.toastService.showSuccess(
              "Comment deleted from " + args.dataContext.ip,
              5000
            );
        ...
        this.vms
          ...
          .then((res2: any) => {
            ...
            if (res2.status == "Success") {
              if (this.loggedUser.useToast)
                this.toastService.showSuccess(
                  "Comment added to " + args.dataContext.ip,
                  5000
                );
              this.vms.setNeedRefresh(true);
              const dataContext = args.dataContext;
    ...
    this.vms
      ...
      .then((res2: any) => {
        ...
        if (res2.status == "Success") {
          if (this.loggedUser.useToast)
            this.toastService.showSuccess(
              args.dataContext.ip + " released from use.",
              5000
            );
          this.vms.setNeedRefresh(true);
          const dataContext = args.dataContext;
        ...
        if (res.status == "FAILED" && res.message == "No record found!") {
          ...
          if (this.loggedUser.useToast) {
            this.toastService.showDanger(
              "No record found! Please add relationships to Action > Relationships",
              10000
            );
          } else {
            this.openDialog(
        ...
        this.vms
          ...
          .then((res2: any) => {
            ...
            if (res2.status == "Success") {
              if (this.loggedUser.useToast)
                this.toastService.showSuccess(
                  args.dataContext.ip + " assigned to " + res + ".",
                  5000
                );
              this.vms.setNeedRefresh(true);
              const dataContext = args.dataContext;
  ...
  defineGrid(gridStatePresets?: GridState) {
    ...
    const ipcellFormatter: Formatter<any> = (
      _row,
      _cell,
      value,
      colDef,
    ...
    /* Custom Formatter for cells to check snapshot count */
    const cellFormattercustomToolTip: Formatter<any> = (
      _row,
      _cell,
      value,
      colDef,
    ...
    /* Custom Formatter for cells to check snapshot count */
    const snapshotFormatter: Formatter<any> = (
      _row,
      _cell,
      value,
      colDef,
    ...
    const cellFormatter: Formatter<any> = (_row, _cell, value, colDef, vm) => {
      ...
      if (
        ...
        return {
          text: `<div style='text-align:center;width:auto;color:#000;' > <span  class='  ${this
            .loggedUser.enableBadgeForSnapWarning
            ? "badge bg-warning"
            : this.loggedUser.disableColorForSnapWarning
              ? ""
              : "badge warnSnapshot"}'>${value}</span></div>`,
          toolTip: value,
        };
      } else if (vm.snap_count > this.properties.alertSnapshot) {
        return {
          text: `<div style='text-align:center;width:auto;color:#000;' ><span style='text-align:center;color:#000;' class=' ${this
            .loggedUser.enableBadgeForSnapWarning
            ? "badge bg-danger"
            : this.loggedUser.disableColorForSnapWarning
              ? ""
              : "badge alertSnapshot"}'>${value}</span></div>`,
          toolTip: value,
        };
      } else {
        return {
    ...
    /* Custom Formatter for cells to check snapshot count */
    const ownerCellFormatter: Formatter<any> = (
      _row,
      _cell,
      value,
      colDef,
      ...
      if (
        ...
        return {
          text: `<div style='text-align:center;text-transform:capitalize;width:auto;'> <span   class="badge ${this
            .loggedUser.enableBadgeForSnapWarning
            ? "bg-warning"
            : this.loggedUser.disableColorForSnapWarning
              ? ""
              : "warnSnapshot"}">${value}</span></div>`,
          toolTip: value,
        };
      } else if (vm.snap_count > this.properties.alertSnapshot) {
        return {
          text: `<div style='text-align:center;text-transform:capitalize;width:auto;'><span style='text-align:center' class=' badge ${this
            .loggedUser.enableBadgeForSnapWarning
            ? "bg-danger"
            : this.loggedUser.disableColorForSnapWarning
              ? ""
              : "alertSnapshot"}' >${value}</span></div>`,
          toolTip: value,
        };
      } else {
        return {
    ...
    var commandListsForContextMenu: any = [
      ...
      },
      {
        ...
        positionOrder: 66,
        action: (_event: any, args: any) =>
          this.openRelatedVMSGraph(_event, args),
      },
      {
        command: "sshTool",
    ...
    const snapshotWarnFormatter: Formatter<any> = (
      _row,
      _cell,
      value,
      colDef,
      ...
      vm.snap_count <= this.properties.alertSnapshot
        ? '<span placement="right"  class="btn  card badge bg-warning mui-warning"><i class="fa fa-warning" aria-label="hidden"></i></span>'
        : ""}
      ${vm.snap_count > this.properties.alertSnapshot
        ? '<span placement="right"  class="btn card badge bg-danger customA mui-danger"><i class="fa fa-warning" aria-label="hidden"></i></span>'
        : ""}
    ...
    if (this.osTypes) {
      ...
      osFilterModel = { model: Filters.input };
    }
    if (this.osVersiontypes) {
      osVersionTypeModel = tempostypefilter;
    } else {
      osVersionTypeModel == { model: Filters.input };
    }

    /* Owner Filter */
    var ownerFilter = {
      collection: this.ownerList,
      customStructure: {
        value: "value",
        label: "owner",
      },
      ...
      gridMenu: {
        ...
        commandItems: [
          ...
          },
          {
            ...
            disabled: false,
            command: "importExcel",
            itemVisibilityOverride: (args: any): boolean => {
        ...
        onCommand: (_e: any, args: any) => {
          if (args.command === "resetGrid") {
            ...
            this.excelExportService.exportToExcel({
              filename:
                "VM_List" + "_" + new Date().toTimeString().replace(" ", "_"),
  ...
  renderAngularComponentForSnapshotCell(
    cellNode: HTMLElement,
    row: number,
    dataContext: any,
    colDef: Column
    ...
    if (colDef.params.component) {
      ...
      Object.assign(componentOutput.componentRef.instance, {
        data: dataContext,
        parentObject: this,
        functionName: "openSnapshots",
        template: dataContext.snap_count,
        functionParameter1: dataContext.hostname,
      });
  ...
  renderAngularComponentForIPCell(
    cellNode: HTMLElement,
    row: number,
    dataContext: any,
    colDef: Column
    ...
    if (colDef.params.component) {
      ...
      if (
        ...
        Object.assign(componentOutput.componentRef.instance, {
          data: dataContext,
          template: `<span  class=' badge ${this.loggedUser
            .enableBadgeForSnapWarning
            ? "bg-warning"
            : this.loggedUser.disableColorForSnapWarning
              ? ""
        ...
        Object.assign(componentOutput.componentRef.instance, {
          ...
          template: `<span  class=' badge ${this.loggedUser
            ...
            : this.loggedUser.disableColorForSnapWarning
              ? ""
              : "alertSnapshot"}'>${dataContext.ip}</span>`,
          type: "alert",
          uiprop: this.properties,
        });
      } else {
      ...
      .then((res: any) => {
        ...
        if (res.status == "Failed" || res.status == false) {
          ...
          this.openSnapshotDialog(
            { snapshots: res, parentObject: this, hostname: hostname },
            null
          );
        }
      })
  ...
  tooltipFormatter(
    ...
    if (
      typeof dataContext.comment != "undefined" &&
      dataContext.comment.toString().trim().length != 0 &&
      dataContext.comment.toString().trim().length > 15
    )
      return {
