 ...
 * @desc Dynamic Object App Edit Record Component
 */
...
@Component({
  selector: "app-dynamicobjectapp-edit-record",
  templateUrl: "./dynamicobjectapp-edit-record.component.html",
  styleUrls: [ "./dynamicobjectapp-edit-record.component.scss" ],
})
export class DynamicObjectAppEditRecordComponent implements OnInit {
  ...
  filterAttributes(list: any): any {
    ...
    for (var item in list) {
      if (
        list[item].type.value != "autokey" &&
        (list[item].isPrimaryKey.value != true ||
          list[item].isPrimaryKey.value.toString() != "true")
      )
  ...
  onSubmit() {
    ...
    var reqBodyToBeSent: any = {
      form: this.registerForm.getRawValue(),
      metadata: metaDataTobeSent,
    };

    var _promise = this._client.post(
      "api/dynamicobjects/" + this.app + "/update",
      reqBodyToBeSent,
      httpOptions
    );
    _promise
