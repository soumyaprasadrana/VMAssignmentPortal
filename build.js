...
async function copySaaSFiles() {
  ...
  if (fs.existsSync("./build/docker/saas/portal-ui")) {
    ...
    fs.copyFile(
      "./build/VMAssignmentPortal.zip",
      "./build/docker/saas/portal-ui/files/VMAssignmentPortal.zip",
      (err) => {
        if (err) {
          console.log(err);
          console.log(
            "Unable to copy resorce: VMAssignmentPortal.zip to saas directory."
          );
        } else {
          console.log(
            "Rousource: VMAssignmentPortal.zip copied to saas directory."
          );
  ...
  if (fs.existsSync("./build/docker/saas/portal-api")) {
    ...
    fs.copyFile(
      "./VMManagementPortalAPI.war",
      "./build/docker/saas/portal-api/files/VMManagementPortalAPI.war",
      (err) => {
        if (err) {
          console.log(err);
          console.log(
            "Unable to copy VMManagementPortalAPI.war to saas directory."
          );
        } else {
          console.log(
            "Rousource: VMManagementPortalAPI.war copied to saas directory."
          );
        }
      }
    );
    fs.copyFile(
      "./wait-for-it.sh",
      "./build/docker/saas/portal-api/files/wait-for-it.sh",
      (err) => {
        if (err) {
          console.log(err);
  ...
  if (fs.existsSync("./build/docker/saas/portal-db")) {
    ...
    fs.copyFile(
      "./portaldb.sql",
      "./build/docker/saas/portal-db/files/portaldb.sql",
      (err) => {
        if (err) {
          console.log(err);
          console.log(
            "Unable to copy resorce: ./portaldb.sql to docker directory."
          );
        } else {
...
async function createSaaSFiles() {
  ...
  const portal_db_docker_file_data =
    "FROM mysql:5.7 " +
    "\n" +
  ...
  createFile(
    ...
    "Dockerfile",
    portal_db_docker_file_data
  );
  createFile(
    "./build/docker/saas/portal-api",
    "Dockerfile",
    portal_api_docker_file_data
  );
  ...
  npm_install.on("close", (code) => {
    ...
    if (code == 0) {
      ...
      ng_build.on("close", (code, res) => {
        if (code == 0) {
          console.log(
            `Angular application build success for type ${buildType}`
          );
          const server_npm_install = spawn(
            /^win/.test(process.platform) ? "npm.cmd" : "npm",
            [ "install" ],
            {
          ...
          server_npm_install.on("close", (code) => {
            if (code == 0) {
              ...
              if (
                extractArchive("./spa.zip", "./server/") &&
                extractArchive("./functions.zip", "./server/api/functions/")
              ) {
                ...
                console.log(
                  "Unable to extract Single Page Apps! Try extracting manually isinde server folder!"
                );
                process.exit(1);
              }
            } else {
          ...
          console.log(
            `build failed with error please check console log.Exit Code:${code},error:${res}`
          );
        }
      });
    } else {
