...
module.exports = {
    ...
    assignMultipleVMS: function(req, res, next) {
        ...
        _client.post(_client.getHttpPostOptions(req, config.assign_multiple_vms_rest_path), req, res, next);
    },
    releaseMultipleVMS: function(req, res, next) {
        logger.info("releaseMultipleVMS:req.body>", req.body);
        _client.post(_client.getHttpPostOptions(req, config.release_multiple_vms_rest_path), req, res, next);
    },
    assignVM: function(req, res, next) {
        ...
        _client.post(_client.getHttpPostOptionsWithCustomQS(req, config.assign_vm_rest_path, {
            AUTH: req.user.auth,
    ...
    releaseVM: function(req, res, next) {
        ...
        _client.post(_client.getHttpPostOptionsWithCustomQS(req, config.release_vm_rest_path, {
            AUTH: req.user.auth,
            JSESSIONID: req.user.jsession,
            ip: req.body.params.ip,
        }), req, res, next);
    ...
    updateMultipleVMS: function(req, res, next) {
        if (req.body.params.fields.ngxos) {
            ...
            req.body.params.fields.os_ver = req.body.params.fields.ngxos.replace(req.body.params.fields.ngxos.split(" ")[0], "");
    ...
    getAdditional: function(req, res, next) {
        ...
        var httpOptions = {
            uri: config.apiBase + '/' + config.apiContextRoot + config.vm_additional_data + '/' + ip,
            headers: _client.getStaticHeaders(req),
            qs: _client.getStaticQueryParam(req),
            jar: _client.getStaticCookieJar(req)
        }
    ...
    deleteVM: function(req, res, next) {
        ...
        logger.info(req.body);
        _client.post(_client.getHttpPostOptions(req, config.delete_vm_rest_path), req, res, next);
    },
    addComment: function(req, res, next) {
        var ip = req.params['ip']
        req.body.params.ip = ip;
        logger.info(req.body);
        _client.post(_client.getHttpPostOptionsWithCustomQS(req, config.add_comment_to_vm_rest_path, {
            AUTH: req.user.auth,
            JSESSIONID: req.user.jsession,
            ip: req.body.params.ip,
    ...
    upload: async function(req, res, next) {
                ...
                request_promise.get(httpOptions).then(function(body) {
                    ...
                    if (typeof req.user.activeUser.permissions.is_admin != 'undefined' && (req.user.activeUser.permissions.is_admin == 1 || req.user.activeUser.permissions.is_admin == '1') && teamList.length > 0) {
                        for (var item in xlsData) {
                            console.log(" validation check for :: " + xlsData[item]['IP Address']);
                            if (!teamList.includes(xlsData[item]['Team'])) {
                                resolve({ status: false, message: "Invalid team for vm with ip " + xlsData[item]['IP Address'] });
                            }
                            if (xlsData[item]['Team'] == null || xlsData[item]['Team'] == '' || xlsData[item]['IP Address'] == null || xlsData[item]['IP Address'] == '' || xlsData[item]['Hostname'] == null || xlsData[item]['Hostname'] == '' || xlsData[item]['OS'] == null || xlsData[item]['OS'] == '' || xlsData[item]['OS Version'] == null || xlsData[item]['OS Version'] == '') {
                                resolve({ status: false, message: "Mandatory field missing for vm with ip " + xlsData[item]['IP Address'] });
                            }
                        }
                        resolve({ status: true });
                    }
        ...
        if (!req.files && !req.file) {
            return res.status(400).json({ status: "Failed", message: "Please upload a flie!", nofile: true })
        }
        var file = null;
        if (req.file)
            file = req.file;
        if (req.files)
            file = req.files.file0;
        try {
            if (file != null) {
                var data = file.data;
                var arr = new Array();
                ...
                if (!checkMandatoryCols(columnHeaders)) {
                    return res.status(400).json({ status: "Failed", message: "Cloumn Headers validation failed!", mandatoryHeaders: ["IP Address", "Hostname", "OS", "OS Version", "Team"], otherAllowedHeaders: ["RAM", "Owner", "Group", "Comment", "Assignee"] })
                }
                let xlsData = XLSX.utils.sheet_to_json(worksheet, {
                    raw: true
                })
                if (xlsData.length == 0) {
                    return res.status(400).json({ status: "Failed", message: "No recod found!", noRecord: true })
                }
                var unique = Object.values(xlsData.reduce((r, o) => {
                    r[o['IP Address']] = r[o['IP Address']] || o;
                    return r;
                }, {}));
                console.log(unique);
                if (unique.length != xlsData.length) {
                    return res.status(400).json({ status: "Failed", message: "Duplicate IP found ! IP Address should be unique for each vm!", duplicateIP: true })
                }
                checkDataValidation(xlsData).then((result) => {
                    console.log(result);
                    if (!result.status) {
                        return res.status(400).json({ status: "Failed", message: result.message, dataValidationFailed: true })
                    }
                    req.body.params = {};
