...
module.exports = {
      ...
      request_promise(extServerOptions, function(error, response, body) {
        ...
        try {
          ...
          if (body.status) {
            ...
            tempUser.enableSnapshotManagements =
              config.enableSnapshotManagements;
            tempUser.enableRichTextForVMComment =
              config.enableRichTextForVMComment;
            tempUser.enableBadgeForSnapWarning =
              config.enableBadgeForSnapWarning;
            tempUser.disableColorForSnapWarning =
              config.disableColorForSnapWarning;
    ...
    try {
      ...
      var extServerOptions = {
        uri:
          ...
          "/auth/ensureAuth/" +
          req.session.passport.user.jsession +
          "/" +
          req.session.passport.user.auth,
        method: "POST",
        headers: {
          "Content-Type": "text/html",
  ...
  post: function(httpOptions, req, res, next) {
    try {
      ...
      request_promise
        .post(httpOptions.uri, httpOptions)
        .then(function(body) {
        ...
        .catch(function(error) {
          logger.info(fun + "- POST request to " + httpOptions.uri);
          logger.debug(
            fun + "- POST request httpOptions: " + JSON.stringify(httpOptions)
          );
  ...
  /*
     * Unauthorized Get Call to Rest API
     * @httpOptions
     * @req
     * @res
     * @next
     */
  getunauthorized: function(httpOptions, req, res, next) {
    try {
      const fun = "client.js :-: getunauthorized";
      var session = req.session;
      request_promise
        .get(httpOptions)
        .then(function(body) {
  ...
  /*
     * Unauthorized POST Call to Rest API
     * @httpOptions
     * @req
     * @res
     * @next
     */
  postunauthorized: function(httpOptions, req, res, next) {
    try {
      const fun = "client.js :-: postunauthorized";
      request_promise
        .post(httpOptions.uri, httpOptions)
        .then(function(body) {
          logger.info(fun + "- POST request to " + httpOptions.uri);
  ...
  /*
     * Get unauthorized static http post options for a Rest API call
     * @req
     */
  getUnauthorizedHttpPostOptions: function(req, apiPath) {
    var httpOptions = {
      uri: config.apiBase + "/" + config.apiContextRoot + apiPath,
      headers: this.getStaticHeaders(req),
      json: false,
