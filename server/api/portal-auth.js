...
module.exports = {
  ...
  custStrategyConfigure: function() {
    ...
    passport.use(
      "portal-auth",
      new passportCustom(function(req, callback) {
        ...
        if (
          ...
          require("./client").authenticateToAPI(
            req.headers.Authorization || req.headers.authorization,
  ...
  authenticate: function(req, res, next) {
    ...
    passport.authenticate("portal-auth", function(error, user) {
      logger.debug(
        "Error:" + JSON.stringify(error) + "User:" + JSON.stringify(user)
      );
      if (error) {
  ...
  ensureAuthenticated: function(req, res, next) {
    ...
    res.redirect("/portal/login?target=" + req.originalUrl);
    next();
    return;
  },
